...
export default function ProjectDetailPage() {
  ...
  const projectedEnd =
    draft.planned_start && draft.est_duration_days && !draft.planned_end
      ? endAfterWorkingDays(
          draft.planned_start,
          Number(draft.est_duration_days),
          cal,
        )
      : null;
  const impliedDays =
  ...
  async function save(e: FormEvent) {
    ...
    const row = {
      ...
      est_duration_days: draft.est_duration_days
        ? Number(draft.est_duration_days)
        : null,
      use_company_calendar: draft.use_company_calendar,
      custom_work_days: draft.use_company_calendar
        ? null
        : draft.custom_work_days,
      custom_start_time: draft.use_company_calendar
        ? null
        : draft.custom_start_time || null,
      custom_end_time: draft.use_company_calendar
        ? null
        : draft.custom_end_time || null,
      notes: draft.notes,
    };
    if (isNew) {
  ...
  return (
    <section>
      ...
      <form id="project-form" onSubmit={save}>
                        ...
                        onChange={() =>
                          set(
                            ...
                            on
                              ...
                              : WORK_TYPE_ORDER.filter(
                                  (x) =>
                                    x === w || draft.work_types.includes(x),
                                ),
                          )
                        }
                      />
        ...
        <div className="card section">
          ...
          <p className="section-help">
            Give a planned end date, or a start date plus an estimated duration
            in working days and the end date is figured for you.
          </p>
          <div className="form-grid">
            <label className="field">
            ...
            <div className="field">
              ...
              <div
                style={{ minHeight: 42, display: "flex", alignItems: "center" }}
              >
                {draft.planned_end ? (
                  <span>
      ...
      {!isNew && (
        <PermitsSection
          projectId={Number(id)}
          isTotalDemo={orig.work_types.includes("total")}
        />
      )}
      ...
      <div className="card section">
        ...
        <div className="radio-row" style={{ marginBottom: 12 }}>
          <label
            className={`check-chip ${draft.use_company_calendar ? "on" : ""}`}
          >
            <input
              type="radio"
              name="cal"
        ...
        {draft.use_company_calendar ? (
          <p className="hint">
            {settings ? (
              <>
                <strong>
                  {settings.work_days.map((d) => DAY_LABELS[d]).join(", ")}
                </strong>
                {" · "}
                {t12(settings.default_start_time)} –{" "}
                {t12(settings.default_end_time)}
              </>
            ) : (
          ...
          <>
            ...
            <p className="hint">
              Leave times blank to use the company default times.
            </p>
          </>
        )}
      </div>
      ...
      {!isNew && isAdmin && (
        <div className="card section danger-zone">
          ...
          <p className="section-help">
            Removes the project, its contacts and every assignment on the
            schedule. Prefer setting the status to Closed unless it was entered
            by mistake.
          </p>
...
const PERMIT_STATUS_ORDER: PermitStatus[] = [
  "not_started",
  "requested",
  "complete",
  "not_required",
...
function PermitsSection({
  ...
  async function addStandard() {
    ...
    const rows = TOTAL_DEMO_PERMIT_ITEMS.filter(
  ...
  async function patch(p: ProjectPermit, changes: Partial<ProjectPermit>) {
    ...
    setItems((cur) =>
      cur.map((x) => (x.id === p.id ? { ...x, ...changes } : x)),
    );
    const { error } = await supabase
      .from("project_permits")
      .update(changes)
  ...
  function setStatus(p: ProjectPermit, status: PermitStatus) {
    ...
    const status_date =
      status === "requested" || status === "complete"
        ? (p.status_date ?? today)
        : p.status_date;
  ...
  return (
    <div className="card section">
      <div className="page-head" style={{ marginBottom: 4 }}>
        ...
        {items.length > 0 && (
          <span
            className={`status-pill ${doneNeeded === needed ? "status-active" : "status-in_permitting"}`}
          >
      ...
      <p className="section-help">
        Everything that has to be cleared before the job can start. Tap a status
        to change it.
      </p>
      ...
      {loaded && items.length === 0 && (
        <div
          className="empty"
          style={{ textAlign: "left", padding: "4px 0 12px" }}
        >
          {isTotalDemo
      ...
      {items.length > 0 && (
        <ul className="list permit-list">
          {items.map((p) => (
            <li
              key={p.id}
              className={p.status === "not_required" ? "inactive" : ""}
            >
              <div className="grow">
              ...
              <div className="permit-controls">
                ...
                <input
                  ...
                  value={p.status_date ?? ""}
                  onChange={(e) =>
                    patch(p, { status_date: e.target.value || null })
                  }
      ...
      <form className="row" onSubmit={addCustom} style={{ marginTop: 12 }}>
        ...
        {isTotalDemo &&
          items.length > 0 &&
          items.length < TOTAL_DEMO_PERMIT_ITEMS.length && (
            <button
              type="button"
              className="btn"
              disabled={busy}
            ...
            <div className="grow">
              ...
              <div className="sub">
                {[
                  c.company,
                  c.phone && (
                    <a key="p" href={`tel:${c.phone}`}>
                      {c.phone}
                    </a>
                  ),
                  c.email && (
                    <a key="e" href={`mailto:${c.email}`}>
                      {c.email}
                    </a>
                  ),
                ]
...
function friendly(msg: string) {
  if (/planned_end_after_start/.test(msg))
    return "Planned end must be on or after planned start.";
  return msg;
}
